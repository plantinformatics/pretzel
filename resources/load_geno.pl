...
my $data_dir = $ARGV[0] || die "Please provide the directory that stores all the sample ***_GENO_CompleteTable.txt files. \n";
opendir(my $dir_hd, $data_dir) || die "Can't open the directory $data_dir. Please have a check. \n";
my @files = grep {/GENO/} readdir($dir_hd);
closedir($dir_hd);
...
my $query = "INSERT INTO genocomplete (clustergroup, score, geno, locus, sample, marker) VALUES (?,?,?,?,?,?)";
