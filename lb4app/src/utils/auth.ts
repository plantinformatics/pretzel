...
export class AuthUtils {
  ...
  async authorizeBlocksRead(blockIds: Array<string | ObjectIdLike>): Promise<void> {
    ...
    const normalizedIds = blockIds.map(id => id?.toString());
    const blocks = await this.blockRepository.find({where: {id: {inq: normalizedIds}}});
    const blockIdSet = new Set(blocks.map(b => b.id?.toString()).filter(Boolean) as string[]);
    const missing = normalizedIds.filter(id => !blockIdSet.has(id));
    if (missing.length) {
